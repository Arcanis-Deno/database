import { KVDatabase } from '../mod.ts';

// deno run --env-file=./.env -A --unstable-kv ./scripts/upsertApplication.rconf.ts
const schema = KVDatabase.getRemoteConfigurationDatabase();
...
// Get Document
const document = await schema.application.findByPrimaryIndex('applicationId', applicationId);
...
switch (mode) {
  case 'add': {
    ...
    const result = await schema.application.add({
      applicationId,
      publicKey,
      clientId,
      clientSecret,
    ...
    if (result.ok) {
      ...
      const q = await schema.application.find(result.id);
      console.info(q);
  ...
  case 'update': {
    ...
    const result = await schema.application.updateByPrimaryIndex('applicationId', applicationId, update, {
      strategy: 'merge',
    });
    if (result.ok) {
